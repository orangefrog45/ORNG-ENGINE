...
mat3 CalculateTbnMatrixTransform() {
	...
	t = normalize(t - dot(t, n) * n);
	vec3 b = normalize(vec3(vec4(cross(vs_normal, vs_tangent), 0.0)));
...
void main() {
	...
	vec2 adj_tex_coord = vs_tex_coord.xy; // add logic once parallax works

	if (u_terrain_mode) {
		mat3 tbn = CalculateTbnMatrix();
		vec3 mixed_terrain_normal = tbn * vec3(texture(normal_array_sampler, vec3(adj_tex_coord, 0.0)).rgb * 2.0 - 1.0).rgb;
		normal = vec4(normalize(mixed_terrain_normal), 1.f);
		float terrain_factor = clamp(dot(normal.xyz, vec3(0, 1.f, 0)), 0.f, 1.f); // slope
