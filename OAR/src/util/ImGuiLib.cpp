...
void ImGuiLib::ShowTextureEditor(Texture2D* selected_texture, Texture2DSpec& spec) {
	if (ImGui::TreeNode("Texture Editor")) {
		...
		ImGui::Text(selected_texture->GetFilename().c_str());
		...
		if (ImGui::Button("Load")) {
			...
			selected_texture->Load();
		}
		ImGui::TreePop();
	}
}

void ImGuiLib::ShowFileExplorer(std::string& path, std::string& t_entry_name, bool& click_flag) {
	ImGui::GetStyle().ChildBorderSize = 1.f;
	if (ImGui::BeginChild(ImGuiID(&click_flag), ImVec2(800, 250))) {
		ImGui::Text(path.c_str());
		if (path.find('/') != std::string::npos && ImGui::Button(ICON_FA_ANCHOR "BACK")) {
