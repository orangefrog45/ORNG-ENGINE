...
void Quad::Load() {
	...
	GLCall(glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 6, &vertices[0], GL_STATIC_DRAW));
	GLCall(glEnableVertexAttribArray(0));
	GLCall(glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0));

	GLCall(glBindBuffer(GL_ARRAY_BUFFER, m_tex_buffer));
	GLCall(glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec2) * 6, &tex_coords[0], GL_STATIC_DRAW));
