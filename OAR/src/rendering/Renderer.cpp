...
void Renderer::RenderScene(Scene& scene, Quad& quad) {
	...
	m_framebuffer_library.deferred_fb.BindForDraw();
	...
	RendererResources::BindTexture(GL_TEXTURE_2D, m_framebuffer_library.dir_depth_fb.GetDepthMap(), RendererResources::TextureUnits::DIR_SHADOW_MAP);
	RendererResources::BindTexture(GL_TEXTURE_2D_ARRAY, m_framebuffer_library.spotlight_depth_fb.GetDepthMap(), RendererResources::TextureUnits::SPOT_SHADOW_MAP);
	...
	RendererResources::BindTexture(GL_TEXTURE_2D, m_framebuffer_library.deferred_fb.GetGeometryTexture(), RendererResources::TextureUnits::WORLD_POSITIONS);
...
void Renderer::DrawShadowMap(Scene& scene) {
	...
	//DIRECTIONAL LIGHT
	m_framebuffer_library.dir_depth_fb.BindForDraw();
	...
	//SPOT LIGHTS
	m_framebuffer_library.spotlight_depth_fb.BindForDraw();
	auto& lights = scene.GetSpotLights();

	for (unsigned int i = 0; i < lights.size(); i++) {

		m_framebuffer_library.spotlight_depth_fb.SetDepthTexLayer(i); // index 0 = directional light depth map
...
template <typename T> void Renderer::DrawLightingGroups(Scene& scene, T& shader) {
...
template <typename T> void Renderer::DrawReflectGroups(Scene& scene, T& shader) {
	for (auto group : scene.GetGroupMeshEntities()) {
...
template <typename T> void Renderer::DrawMeshWithShader(Scene& scene, MeshData* mesh_data, unsigned int t_instances, T& shader) {
