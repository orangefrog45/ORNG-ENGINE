...
namespace ORNG {
	...
	public:
		...
		void SetPoint(size_t index, const glm::vec4& v);

		void RemovePoint(size_t index);

		[[nodiscard]] unsigned GetNbPoints() const noexcept { return static_cast<unsigned>(points.size()); }

		glm::vec4 GetPoint(size_t index);
		...
		inline static const size_t GPU_STRUCT_SIZE_BYTES = sizeof(glm::vec4) * 8 + sizeof(unsigned);
