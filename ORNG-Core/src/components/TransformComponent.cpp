...
namespace ORNG {
	void TransformComponent::UpdateAbsTransforms() {
		...
		m_abs_pos = lml::vec3(m_transform[3][0], m_transform[3][1], m_transform[3][2]);

		if (m_is_absolute)
			return;

		lml::quat accumulated_parent_rotation = lml::quat{1.0f, 0.0f, 0.0f, 0.0f};
	...
	void TransformComponent::LookAt(lml::vec3 t_pos, lml::vec3 t_up) {
		lml::vec3 euler_angles = lml::degrees(lml::eulerAngles(lml::quatLookAt(lml::normalize(t_pos - m_abs_pos), lml::normalize(t_up))));
	...
	void TransformComponent::RebuildMatrix(UpdateType type) {
		...
		if (m_is_absolute || !p_parent) {
			...
			forward = lml::normalize(lml::vec3(-m_transform[2][0], -m_transform[2][1], -m_transform[2][2]));
			right = lml::normalize(lml::vec3(m_transform[0][0], m_transform[0][1], m_transform[0][2]));
			up = lml::normalize(lml::vec3(m_transform[1][0], m_transform[1][1], m_transform[1][2]));
			...
			lml::vec3 p_s = p_parent->GetAbsScale();
			float inv_parent_scale_det = 1.f / (p_s.x * p_s.y * p_s.z);
			lml::vec3 total_scale = m_scale * p_s;
			lml::vec3 inv_scale{ p_s.y * p_s.z * inv_parent_scale_det, p_s.x * p_s.z * inv_parent_scale_det, p_s.y * p_s.x * inv_parent_scale_det };

			m_transform = lml::mat4_cast(m_orientation);
			...
			forward = m_abs_orientation * lml::vec3{ 0.f, 0.f, -1.f };
			right = m_abs_orientation * lml::vec3{ 1.f, 0.f, 0.f };
			up = m_abs_orientation * lml::vec3{ 0.f, 1.f, 0.f };
		}
