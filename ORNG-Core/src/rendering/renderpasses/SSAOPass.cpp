...
void SSAOPass::Init() {
	m_ssao_shader.AddStage(GL_COMPUTE_SHADER, "res/core-res/shaders/SSAOCS.glsl");
	m_ssao_shader.Init();
	...
	if (!p_gbuffer_pass) {
		...
		mp_depth_tex = &p_gbuffer_pass->depth;
		mp_normal_tex = &p_gbuffer_pass->normals;
	}
}

void SSAOPass::DoPass() {
	GL_StateManager::BindTexture(GL_TEXTURE_2D, mp_normal_tex->GetTextureHandle(), GL_TEXTURE1);
	GL_StateManager::BindTexture(GL_TEXTURE_2D, mp_depth_tex->GetTextureHandle(), GL_TEXTURE2, true);

	const auto& spec = m_ao_tex.GetSpec();
	m_ssao_shader.ActivateProgram();
