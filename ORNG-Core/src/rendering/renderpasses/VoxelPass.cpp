...
// 	m_scene_voxelization_shader.AddVariant(static_cast<unsigned>(VoxelizationSV::MAIN), { "VOXELIZE" }, voxel_uniforms);
//
// 	m_voxel_compute_sv.SetPath(GL_COMPUTE_SHADER, "res/core-res/shaders/VoxelDecrementCS.glsl");
// 	m_voxel_compute_sv.AddVariant(static_cast<unsigned>(VoxelCS_SV::DECREMENT_LUMINANCE), { "DECREMENT_LUMINANCE" }, {});
// 	m_voxel_compute_sv.AddVariant(static_cast<unsigned>(VoxelCS_SV::ON_CAM_POS_UPDATE), { "ON_CAM_POS_UPDATE" }, { "u_delta_tex_coords" });
// 	m_voxel_compute_sv.AddVariant(static_cast<unsigned>(VoxelCS_SV::BLIT), { "BLIT" }, {});
//
// 	m_3d_mipmap_shader.SetPath(GL_COMPUTE_SHADER, "res/core-res/shaders/MipMap3D.glsl");
// 	m_3d_mipmap_shader.AddVariant(static_cast<unsigned>(MipMap3D_ShaderVariants::DEFAULT_MIP), {}, {});
// 	m_3d_mipmap_shader.AddVariant(static_cast<unsigned>(MipMap3D_ShaderVariants::ANISOTROPIC), { "ANISOTROPIC_MIPMAP" }, {});
// 	m_3d_mipmap_shader.AddVariant(static_cast<unsigned>(MipMap3D_ShaderVariants::ANISOTROPIC_CHAIN), { "ANISOTROPIC_MIPMAP_CHAIN" }, { "u_mip_level" });
//
...
// 	float half_cascade_width = static_cast<float>(cascade_width) * 0.5f;
//
...
// 	m_voxel_compute_sv.Activate(static_cast<unsigned>(VoxelCS_SV::DECREMENT_LUMINANCE));
// 	GL_StateManager::DispatchCompute(cascade_width / 4, cascade_width / 4, cascade_width / 4);
// 	glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
//
// 	glViewport(0, 0, 256, 256);
// 	m_scene_voxelization_shader.Activate(static_cast<unsigned>(VoxelizationSV::MAIN));
...
// 	glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
//
// 	const int voxel_mip_ratio = static_cast<int>(m_scene_voxel_tex_c0.GetSpec().width / m_voxel_mip_faces_c0.GetSpec().width);
...
//
// 	m_voxel_compute_sv.Activate(static_cast<unsigned>(VoxelCS_SV::ON_CAM_POS_UPDATE));
