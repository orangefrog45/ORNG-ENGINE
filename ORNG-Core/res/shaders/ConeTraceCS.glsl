...
#define CONE_TRACE_SKELETON(x) 	vec3 step_pos = sampled_world_pos + cone_dir * d   ; \
		...
		coord.z /= 6.0; \
		\
		float diam = 2.0 * tan_half_angle * max(d, 0.01);\
		vec4 mip_col = vec4(0);\
		x \
		vec4 voxel = mip_col;\
		if (voxel.a > 0.0) {\
...
vec4 ConeTrace(vec3 cone_dir, float aperture, float mip_scaling, out float dist_travelled) {
	...
	while (col.a < 0.95 && current_cascade == 0 ) {
		CONE_TRACE_SKELETON(
		float mip = min(log2(diam / mip_scaling ), 6.0);
		...
		// if (cam_dist > 20.f) {
		// 	float mip2 = clamp(log2(2.0 * diam / mip_scaling ) - 1.0, 0.0, 5.0);
	...
	while (col.a < 0.95 && current_cascade == 1) {
		CONE_TRACE_SKELETON(
		float mip = clamp(log2(2.0 * diam / mip_scaling ) - 1.0, 0.0, 5.0);
