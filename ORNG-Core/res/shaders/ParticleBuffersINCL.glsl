...
#define EMITTER ubo_particle_emitters.emitters[ubo_particles.particles[index].emitter_index]

void InitializeParticle(highp uint index) {

    float az = rnd(vec2(ubo_particle_transforms.transforms[index].pos.z, ubo_particle_transforms.transforms[index].pos.x)) * 2.0 * PI;
    float pol = rnd(vec2(ubo_particle_transforms.transforms[index].pos.y, -ubo_particle_transforms.transforms[index].pos.z)) * EMITTER.spread;
    ...
    vec3 spawn_pos = EMITTER.pos.xyz + mat3(EMITTER.rotation_matrix) * (vec3(rnd(vec2(index, ubo_common.delta_time)), rnd(vec2(az, index)), rnd(vec2(ubo_particle_transforms.transforms[index].pos.x, ubo_common.delta_time))) *  EMITTER.spawn_extents_vel_scale_max.xyz);


    #ifdef FIRST_INITIALIZATION
        // Give initial offset to life to factor in spawn delay
        ubo_particles.particles[index].velocity_life = vec4(vel, (index - EMITTER.start_index) * EMITTER.spawn_cooldown);

        ubo_particle_transforms.transforms[index].scale = vec4(0);
    #else
        ubo_particles.particles[index].velocity_life.xyz = vel;
        ubo_particles.particles[index].velocity_life.w += EMITTER.lifespan;

        ubo_particle_transforms.transforms[index].scale = vec4(1);

    #endif


    ubo_particle_transforms.transforms[index].pos.xyz = spawn_pos;
...
void OnEmitterDelete(uint emitter_index) {
    for (uint i = emitter_index + 1; i < ubo_particle_emitters.emitters.length(); i++) {
      ...
      for (uint i = ubo_particle_emitters.emitters[emitter_index].start_index + ubo_particle_emitters.emitters[emitter_index].num_particles; i < ubo_particles.particles.length(); i++) {
